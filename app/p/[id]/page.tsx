  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 py-10">
      <header className="space-y-2 border-b border-neutral-200 pb-6">
        <p className="text-xs font-medium uppercase tracking-wide text-neutral-500">Proposal</p>
      ...
      <article className="prose prose-neutral max-w-none">
        {Object.entries(proposal.content as Record<string, string>).map(([key, val]) => (
          <section key={key} className="mb-8">
            ...
            <p className="whitespace-pre-wrap text-neutral-700">{val}</p>
          </section>
        ))}
      </article>
      <footer className="rounded-xl border border-neutral-200 bg-white p-6 text-sm text-neutral-600">
        Signing and payment UI hook to <code className="rounded bg-neutral-100 px-1">/api/sign-proposal</code> and
        Stripe Checkout.
      </footer>
    </div>
  );
